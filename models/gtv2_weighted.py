...
class head_attention_weighted(nn.Module):
    ...
    def forward(self, x, e):
        # 1) Cross-attention from edges to nodes
        x_cross, _ = self.cross_att_edge_to_node(x, e)
        # 2) Cross-attention from nodes to edges
        _, e = self.cross_att_node_to_edge(x, e)
        # 3) Self-attention on nodes
        x_self, _ = self.self_att_node_to_node(x, e)
...
class DiGressNet_GTv2_Weighted(nn.Module):
